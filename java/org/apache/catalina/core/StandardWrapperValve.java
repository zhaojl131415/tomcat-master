...
final class StandardWrapperValve
    ...
    public final void invoke(Request request, Response response)
        ...
        // Allocate a servlet instance to process this request分配一个servlet实例来处理该请求
        try {
            if (!unavailable) {
                /**
                 * 分配一个初始化的servlet实例
        ...
        request.setAttribute(Globals.DISPATCHER_REQUEST_PATH_ATTR,
                requestPathMB);
        // Create the filter chain for this request 创建此请求的过滤器链
        ...
        try {
            if ((servlet != null) && (filterChain != null)) {
                // Swallow output if needed 如果需要吞下输出
                if (context.getSwallowOutput()) {
                    ...
                    if (request.isAsyncDispatching()) {
                        ...
                        /**
                         * 传参为门面模式: RequestFacade/ResponseFacade
                         */
